...
function MessageBubble({ msg, speakingId, onToggleSpeak }) {
  ...
  const bubbleColor = isAdmin
    ...
    : isBot
    ? 'bg-slate-200 text-slate-700 rounded-bl-md border border-slate-300'
    : msg.isSOS
  ...
  return (
    <div className={`${wrapper} mb-1.5`}>
      ...
      <div className="flex flex-col gap-1">
        ...
        {!isAdmin && (
          <span className="text-[10px] font-bold text-slate-400 uppercase tracking-wide ml-1">
            {isBot ? 'Qhawaybot' : 'Abuelo'}
            {msg.channel === 'gmail' && (
              <span className="ml-1 text-[9px] bg-red-50 text-red-500 border border-red-200 px-1 py-0.5 rounded-sm">
                Gmail
              </span>
            )}
          </span>
        ...
        <div className={`${bubbleBase} ${bubbleColor}`}>
          ...
          <div className="flex items-center justify-end gap-2 mt-1.5">
            <span
              className={`text-[10px] ${isAdmin ? 'text-blue-200' : 'text-slate-400'}`}
            >
...
export default function Mensajeria() {
  ...
  return (
    <div className="flex flex-col h-full bg-slate-50 font-sans">
      ...
      {/* Header                                                               */}
      {/* ------------------------------------------------------------------ */}
      <div className="bg-white border-b border-slate-100 shadow-xs px-5 py-4 flex items-center justify-between flex-shrink-0">
        <div>
      ...
      {/* ------------------------------------------------------------------ */}
      {gmailBanner && (
        <div className="bg-green-50 border-b border-green-200 px-5 py-2.5 flex items-center justify-between text-xs font-semibold text-green-700">
          <span>
      ...
      {gmailConnected && lastSync && (
        <div className="bg-slate-50 px-5 py-1.5 flex items-center gap-1.5 text-[10px] text-slate-400 font-semibold flex-shrink-0 border-b border-slate-100">
      ...
      {/* ------------------------------------------------------------------ */}
      {activeTab === 'chat' && (
        <div className="bg-white border-t border-slate-100 px-4 py-3 flex items-end gap-3 flex-shrink-0">
          <div className="flex-1 relative">
            <textarea
              ...
              rows={1}
              className="w-full resize-none bg-slate-50 border border-slate-200 rounded-2xl px-4 py-2.5 text-sm text-slate-800 placeholder-slate-400 outline-none focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 transition max-h-32 overflow-y-auto"
          ...
          <button
            ...
            className="w-10 h-10 bg-blue-600 hover:bg-blue-700 disabled:bg-slate-200 disabled:text-slate-400 text-white rounded-2xl flex items-center justify-center transition-all active:scale-95 flex-shrink-0 cursor-pointer"
          >
